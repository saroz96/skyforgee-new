...
const EditJournalVoucher = () => {
    ...
    useEffect(() => {
        const fetchJournalData = async () => {
            try {
                setLoading(true);
                const response = await api.get(`/api/retailer/journal/${id}`);
                const { data } = response;
                ...
                const maxLength = Math.max(
                    data.data.journal.debitAccounts.length,
                    data.data.journal.creditAccounts.length
                );

                // Pair debit and credit entries in the same row
                for (let i = 0; i < maxLength; i++) {
                    const debitAccount = data.data.journal.debitAccounts[i];
                    const creditAccount = data.data.journal.creditAccounts[i];
                ...
                // Populate form data
                setFormData({
                    billDate: data.data.journal.date ? data.data.journal.date : new Date().toISOString().split('T')[0],
                    nepaliDate: new NepaliDate(data.data.journal.date).format('YYYY-MM-DD') || currentNepaliDate,
                    description: data.data.journal.description || '',
                    entries: entries
                });

                setLoading(false);
            } catch (err) {
                setError(err.response?.data?.message || 'Failed to load journal voucher');
                setLoading(false);
            }
        };
    ...
    //         setNotification({
    //             show: true,
    //             message: err.response?.data?.message || 'Failed to update journal voucher',
    //             type: 'error'
    //         });
    //     } finally {
    //         setIsSaving(false);
    ...
    const handleCancelVoucher = async () => {
        if (window.confirm("Are you sure you want to cancel this voucher?")) {
            try {
                await api.post(`/api/retailer/journals/cancel/${journal.billNumber}`);
                setNotification({
                    show: true,
    ...
    const handleReactivateVoucher = async () => {
        if (window.confirm("Are you sure you want to reactivate this voucher?")) {
            try {
                await api.post(`/api/retailer/journals/reactivate/${journal.billNumber}`);
                setNotification({
                    show: true,
            ...
            try {
                ...
                for (let i = 0; i < maxLength; i++) {
                    ...
                    entries.push({
                        ...
                        debitAccountName: debitAccount ? `${debitAccount.account.uniqueNumber} - ${debitAccount.account.name}` : '',
                        debitAmount: debitAccount ? debitAccount.debit : '',
                        debitOriginalAmount: debitAccount ? debitAccount.debit : 0, // Store original amount
                        creditAccountId: creditAccount ? creditAccount.account._id : '',
                        creditAccountName: creditAccount ? `${creditAccount.account.uniqueNumber} - ${creditAccount.account.name}` : '',
                        creditAmount: creditAccount ? creditAccount.credit : '',
                        creditOriginalAmount: creditAccount ? creditAccount.credit : 0 // Store original amount
                    });
                }
    ...
    if (!journal) return <div className="alert alert-danger mt-5">Journal voucher not found</div>;
            ...
            <div className="container mt-4 wow-form">
                <div className="card shadow-lg p-4 animate__animated animate__fadeInUp">
                    <div className="card-header bg-primary text-white">
                        <div className="row">
                            <div className="col-6">
                                <h5 className="card-title mb-0">Edit Journal Voucher</h5>
                            </div>
