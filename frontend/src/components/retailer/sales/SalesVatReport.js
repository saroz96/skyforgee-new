...
const SalesVatReport = () => {
    ...
    const [data, setData] = useState(() => {
        if (draftSave && draftSave.salesVatData) {
            return draftSave.salesVatData;
        }
        return {
            company: null,
    ...
    // Save draft data
    useEffect(() => {
        if (data.salesVatReport.length > 0 || data.fromDate || data.toDate) {
            setDraftSave({
                ...draftSave,
                salesVatData: data
            });
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const response = await api.get('/api/retailer/sales-vat-report', { params });
                setData({
                    ...response.data.data,
                ...
                setError(err.response?.data?.message || 'Failed to fetch VAT report');
            } finally {
                setLoading(false);
    ...
    //             report.panNumber?.toLowerCase().includes(searchQuery.toLowerCase())
    //         );
    //     });
    ...
    const handlePrint = () => {
        ...
        const formatDisplayDate = (dateString) => {
            ...
            if (data.companyDateFormat === 'nepali') {
                // For Nepali dates, you might want to use a different formatting approach
        ...
        <div class="print-header">
            ...
            <p>
                ...
                <h2 class="report-title">Sales Book</h2>
            </p>
            <hr>
        </div>
    `;
    ...
    <html>
        <head>
            <title>Sales VAT Report - ${getMonthName(data.fromDate, data.companyDateFormat)} ${getYear(data.fromDate)} - ${data.currentCompanyName || 'Company Name'}</title>
        </head>
        <body>
            ${tableContent}
            <script>
        ...
        <div className="container-fluid">
            ...
            <div className="card shadow">
                <div className="card-header bg-white py-3">
                    <h1 className="h3 mb-0 text-center text-primary">Sales VAT Report</h1>
                </div>
                ...
                <div className="card-body">
                    ...
                    {data.salesVatReport.length === 0 ? (
                        <div className="alert alert-info text-center py-3">
                            ...
                            Select date range and generate report to view VAT data
                        </div>
                    ) : (
                        <>
                                ...
                                <table className="table table-hover">
                                    <thead className="table-light">
                                        <tr>
                                            ...
                                            <th className="text-end">Non-VAT Sales</th>
                                    ...
                                    <tbody ref={tableBodyRef} className="table-data-rows">
                                        {filteredReports.map((report, index) => (
                                            ...
                                            >
                                                <td>{new Date(report.date).toISOString().split('T')[0]}</td>
                                                <td>{report.billNumber}</td>
                                                <td>{report.accountName || 'Cash Sale'}</td>
                                                <td>{report.panNumber}</td>
